"""
04 - Advanced Hands-On: Multi-Agent Systems & Orchestration
=============================================================
Exercise 1: SequentialAgent — Content pipeline (Research → Write → Edit)
Exercise 2: ParallelAgent + Fan-In — Multi-perspective analysis
Exercise 3: LoopAgent — Self-correcting code generator
"""
...
# ============================================================
# EXERCISE 1: SequentialAgent — Content Pipeline
# ============================================================
# Data flows: researcher → writer → editor via output_key + {placeholder}

async def exercise_sequential():
    print("=" * 60)
    print("EXERCISE 1: SequentialAgent — Content Pipeline")
    print("=" * 60)

    researcher = LlmAgent(
        name="researcher",
        model=MODEL,
        instruction="Research the topic and provide 3-4 key facts. Topic: {topic}. Be concise, bullet points only.",
    ...
    writer = LlmAgent(
        name="writer",
        model=MODEL,
        instruction="""Write a short paragraph (3-4 sentences) based on these research findings:
{research_findings}
...
Make it engaging and informative.""",
    ...
    editor = LlmAgent(
        name="editor",
        model=MODEL,
        instruction="""Edit this draft for clarity. Fix any issues. Keep it concise:
{draft}
...
Realistic view: {realist_view}""",
    ...
    # Full pipeline: parallel → synthesize
...
# ============================================================
# EXERCISE 3: LoopAgent — Self-Correcting Code Review
# ============================================================
...
If ANY criteria are NOT met, list the specific issues to fix. Do NOT call exit_loop.""",
    ...
    # Loop: coder → reviewer → (repeat if needed)
...
async def main():
    await exercise_sequential()
    await exercise_parallel()
    await exercise_loop()

    print("\n" + "=" * 60)
    print("All advanced exercises complete!")
    print("=" * 60)
    print("""
Patterns demonstrated:
  1. SequentialAgent  — Pipeline with output_key data flow
  2. ParallelAgent    — Fan-out (concurrent) + Fan-in (synthesize)
  3. LoopAgent        — Self-correction loop with exit_loop tool
""")
